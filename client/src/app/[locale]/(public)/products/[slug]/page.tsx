...
const fetchSeo = async (slug: string) => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/seo/findByPage/${slug}`,
      {
        headers: {
...
export async function generateMetadata({
  ...
  if (seo?.og_image) {
    metadata.openGraph = {
      ...
      url: `${process.env.NEXT_PUBLIC_URI}/${currentLocale}/products/${slug}`,
      images: [
        {
          url: seo.og_image,
          width: 1200,
...
const SingleProductPage: React.FC<SingleProductPageProps> = ({
  ...
  return <SingleProduct locale={locale} slug={slug} id={String(id)} />;
};
