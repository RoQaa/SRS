...
const fetchSeo = async () => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/seo/findByPage/video-gallery`,
      {
        headers: {
...
export async function generateMetadata({
  ...
  const defaultMetadata = {
    title: currentLocale === "en" ? "Video Gallery" : "معرض الفيديو",
    description:
      currentLocale === "en"
        ? "Explore our collection of videos showcasing our projects and services."
        : "استعرض مجموعة الفيديوهات التي تعرض مشاريعنا وخدماتنا.",
    keywords:
      currentLocale === "en"
        ? "video gallery, videos, projects, services"
        : "معرض الفيديو, فيديوهات, مشاريع, خدمات",
  };
  ...
  if (seo?.og_image) {
    metadata.openGraph = {
      ...
      url: `${process.env.NEXT_PUBLIC_URI}/${currentLocale}/video-gallery`,
      images: [
        {
          url: seo.og_image,
          width: 1200,
