...
const fetchSeo = async () => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/seo/findByPage/projects`,
      {
        headers: {
...
export async function generateMetadata({
  ...
  // Default metadata for the Projects page if fetching fails
  const defaultMetadata = {
    title: currentLocale === "en" ? "Our Projects" : "مشاريعنا",
    description:
      currentLocale === "en"
        ? "Explore our diverse portfolio of projects showcasing our expertise and quality."
        : "استعرض مشاريعنا المتنوعة التي تبرز خبراتنا وجودتنا.",
    keywords:
      currentLocale === "en"
  ...
  if (seo?.og_image) {
    metadata.openGraph = {
      ...
      url: `${process.env.NEXT_PUBLIC_URI}/${currentLocale}/projects`, 
      images: [
        {
          url: seo.og_image,
          width: 1200,
