...
const ScopeMainImageUpload = () => {
  const [files, setFiles] = useState<ExtFile[]>([]);
  const dispatch = useAppDispatch();

  // Handle file changes and dispatch mainImg
  const updateFiles = (incomingFiles: ExtFile[]) => {
    setFiles(incomingFiles);

    const validFiles = incomingFiles.filter((file) => file.valid === true);
    if (validFiles.length > 0) {
      dispatch(setFormValue({ name: "mainImg", value: validFiles[0].file }));
    } else {
      dispatch(setFormValue({ name: "mainImg", value: "" }));
    }
  };
  ...
  return (
    <div className="sidebar-body">
      <div className="main-image-upload">
        <p>
          Main Image<span className="txt-danger"> *</span>
        </p>
        <Dropzone
...
export default ScopeMainImageUpload;
