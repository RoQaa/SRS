...
const UploadImage = ({
  ...
  const updateThumbnail = (file: ExtFile) => {
    if (file.file instanceof File) {
      ...
      console.error("Invalid file provided for thumbnail.");
    }
  };
  ...
  useEffect(() => {
    const loadThumbnail = async () => {
      if (oldThumbnail) {
        try {
          const fileName = getFileNameFromUrl(`${process.env.NEXT_PUBLIC_API_URL}/${oldThumbnail}`);
          const file = await convertUrlToFile(`${process.env.NEXT_PUBLIC_API_URL}/${oldThumbnail}`, fileName);
