...
const UploadImage = ({ setFieldValue, oldThumbnail }: UploadImageProps) => {
  ...
  const updateThumbnail = (file: ExtFile) => {
    if (file.file instanceof File) {
      setThumbnail(file);
      setFieldValue("backgroundImg", file.file);
    } else {
  ...
  return (
    <div className="sidebar-body mt-4">
      <div className="product-upload">
        <p>
          Middle Section Background Image<span className="txt-danger"> *</span>
        </p>
        <Dropzone
          onChange={(files) => {
